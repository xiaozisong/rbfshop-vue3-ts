...
const orderInfo = ref<OrderPayInfo>()
const getOrderInfo = async () => {
  const res = await http.get<ApiObjRes<OrderPayInfo>>('/member/order/' + route.query.id)
  orderInfo.value = res.data.result
  const { count, start } = useCountDown(res.data.result.countdown)
  start()
  watch(() => count.value, () => {
    setTimeout(() => {
...
const formatTime = (val: any) => {
  let min = (val / 60).toString().padStart(2,'0')
  let sec = (val % 60).toString().padStart(2,'0')
  return `${parseInt(min)}分${parseInt(sec)}秒`
}
// 设置支付宝支付地址
